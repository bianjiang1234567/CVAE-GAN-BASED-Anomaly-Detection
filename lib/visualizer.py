...
class Visualizer():
    ...
    def plot_current_errors(self, epoch, counter_ratio, errors):
        ...
        ax = plt.subplot(111)
        ax.plot(self.plot_data['X'],self.plot_data['Y'])
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        ax.legend(self.plot_data['legend'], numpoints=1, fancybox=True)
        if self.opt.dataset == "cifar10":
            figure.savefig("./cifarloss_{}.pdf".format(self.opt.anomaly_class))
        if self.opt.dataset == "mnist":
            figure.savefig("./mnistloss_{}.pdf".format(self.opt.anomaly_class))
        if self.opt.dataset == "caltech-256-image-dataset":
            figure.savefig("./caltechloss_{}{}.pdf".format(self.opt.num_train,self.opt.number[0]))
        plt.close()
        #print("saving!!!!!!")

        ##
    def plot_current_errors2(self, epoch, counter_ratio, errors):

        if not hasattr(self, 'plot_data') or self.plot_data2 is None:
            self.plot_data2 = {'X': [], 'Y': [], 'legend': list(errors.keys())}
        self.plot_data2['X'].append(epoch + counter_ratio)
        self.plot_data2['Y'].append([errors[k] for k in self.plot_data2['legend']])

        figure = plt.figure()
        ax = plt.subplot(111)
        ax.plot(self.plot_data2['X'], self.plot_data2['Y'])
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        ax.legend(self.plot_data2['legend'], numpoints=1, fancybox=True)
        if self.opt.dataset == "cifar10":
            figure.savefig("./cifarloss2_{}.pdf".format(self.opt.anomaly_class))
        if self.opt.dataset == "mnist":
            figure.savefig("./mnistloss2_{}.pdf".format(self.opt.anomaly_class))
        if self.opt.dataset == "caltech-256-image-dataset":
            figure.savefig("./caltechloss2_{}{}.pdf".format(self.opt.num_train,self.opt.number[0]))
        plt.close()
    ...
    def save_current_images(self, epoch, reals, fakes, fixed, fixed_input):
        ...
        vutils.save_image(fixed_input, '%s/fixed_reals_%03d.png' % (self.img_dir, epoch + 1), normalize=True)
