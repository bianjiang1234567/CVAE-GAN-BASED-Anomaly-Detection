    ...
    if opt.dataset in ['cifar10']:
        ...
        transform = transforms.Compose(
            [
                ...
                transforms.ToTensor(),
                transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))#变成-1到1之间的数字
            ]
        )
        ...
        dataloader = {x: torch.utils.data.DataLoader(dataset=dataset[x],
                                                     ...
                                                     shuffle=shuffle[x],#每个epoach都要改组   test  不改组  shuffle = {'train': True, 'test': False}
                                                     num_workers=int(opt.workers),
                                                     drop_last=drop_last_batch[x]) for x in splits}
        #print("!!!!", dataset['train'].train_data)
        return dataloader
...
def get_cifar_anomaly_dataset(trn_img, trn_lbl, tst_img, tst_lbl, abn_cls_idx=0, manualseed=-1):
    ...
    if manualseed != -1:
        ...
        nrm_trn_len = int(len(idx) * 0.80)#用了20%
